...
def build_summary_prompt(
    ...
    sections.extend([
        ...
        "",
        "**核心原则：**",
        "- **严格基于对话事实**：只输出对话中实际出现的内容，不要猜测、推断或补充",
        "- **宁缺毋滥**：无实质内容的字段留空，不填充凑数",
        "- **逻辑清晰完整**：有内容才写，写就要写清楚",
        "",
        "首先判断会话复杂度：",
        "- simple：内容简单，一两句话能讲清 → 用 simple 模式，只输出简要总结",
        "- complex：内容确实丰富，有决策/调试/多步操作 → 用 complex 模式",
        "",
        ...
        '      "solution": "抽取统一的 error handler 中间件（如对话中未找到方案则写 未解决，不要猜测）",',
        ...
        '      "suggestions": ["先确认跨域策略再选方案（仅写对话中实际提到的）"]',
        '    }',
        "  ],",
        ...
        "",
        "**注意**：",
        "- `overview`、`key_decisions`、`suggestions`、`best_practices` 都是**字符串数组**，",
        "  每个元素是一个独立的字符串，不要合并为单个字符串",
        "- **所有内容必须来自对话原文**，不要添加对话中未提及的信息",
        "- 如果对话中未找到方案，`solution` 写 `未解决`，**不要自己猜测方案**",
        "- 如果某个字段在对话中确实没有对应内容，直接留空，不要编造",
        "",
